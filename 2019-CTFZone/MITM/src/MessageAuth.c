...
static void SHA256_transform(SHA256_CTX *ctx, const unsigned char data[]) {
	...
	for (i = 0; i < 64; ++i) {
		t1 = h + EP1(e) + CH(e,f,g) + constant[i] + m[i];
		t2 = EP0(a) + MAJ(a,b,c);
		h = g;
		g = f;
		f = e;
...
void SHA256_copy_hash_in_mpz(mpz_t hash_out, unsigned char *hash) {
    ...
    for (int8_t i = 0; i < SHA256_BLOCK_SIZE; i++) {
        if (hash[i] == 0) {
             strcpy(hash_hex + 2 * i, "00");
        } 
        else {
            sprintf(chr, "%x", hash[i]);
            if (chr[1] == 0) {
                chr[1] = chr[0];
                chr[0] = '0';
            }
            strncpy(hash_hex + 2 * i, chr, 2);
        }
    } 
	mpz_set_str(hash_out, hash_hex, 16);   
}
...
RSA_key *RSA_key_init(unsigned char *exp, unsigned char *module) {
	...
	mpz_init_set_str(key->exp, exp, 16);
	mpz_init_set_str(key->module, module, 16);
	return key;
}

void RSA_encrypt_decrypt_hash(RSA_key *key, mpz_t input) {
    mpz_powm(input, input, key->exp, key->module);
}